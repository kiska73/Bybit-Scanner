...
async function getSignal(symbol) {
    ...
    try {
        ...
        return {
            ...
            posM: getPosition(
                currentM,
                mData.map(x => x.accountRatio)
            ),

            posT: getPosition(
                currentT,
                tData.map(x => x.topTraderAccountRatio)
            ),
...
async function scan() {
    ...
    for (let i = 0; i < PAIRS_WITH_RATIO.length; i += 5) {
        ...
        for (const data of results) {
            ...
            if (data.posT >= SOGLIA_ALTA && data.posM <= SOGLIA_BASSA)
                type = "⚡ SQUEEZE LONG (BULLISH)";

            else if (data.posT <= SOGLIA_BASSA && data.posM >= SOGLIA_ALTA)
                type = "⚠️ SHORT SQUEEZE (BEARISH)";

            else if (data.posT >= SOGLIA_ALTA && data.posM >= SOGLIA_ALTA)
                type = "🚀 ECCESSO LONG UNANIME";

            else if (data.posT <= SOGLIA_BASSA && data.posM <= SOGLIA_BASSA)
